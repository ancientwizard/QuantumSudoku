    ...
    () => {
      ...
      CellValue.arrayFactory.reverse().forEach(
        (v) => {
          ...
          expect(_c.name).toBe('X0')
          expect(_c.coord).toBe('(0,0)')
          expect(_c.toArray().reverse().toString()).toBe(_a.slice().reverse().toString())
          expect(_c.toArray().length).toBe(_l)
          expect(_c.length).toBe(_l)
          expect(_c.exclude(v)).toStrictEqual(_l!=0)
          expect(_c.toArray().length).toBe(_l-1)
          expect(_c.length).toBe(_l-1)
          expect(_c.isKnown).toStrictEqual(_l==1)
          expect(_c.isUnknown).toStrictEqual(_l>1)
          _a.pop()
          --_l
